...
public class RK4 {
    ...
    private static final double LIFT_COEFF = 0.1; // Magnus lift coefficient
    private static final double BALL_RADIUS_IN = 5.91 / 2.0; // inches
    private static final double BALL_AREA =
        Math.PI * Math.pow(BALL_RADIUS_IN * 0.0254, 2); // m^2
    private static final double BALL_MASS_KG = Kilograms.convertFrom(
        0.5,
        Pounds
    );
    private static final double GRAVITY = 9.81; // m/s^2

    private static final double MAGNUS_COEFF =
        0.5 * AIR_DENSITY * BALL_AREA * LIFT_COEFF;

    private final double dt;
    private final Pose2d targetPose;
    private final Pose2d shooterPose;
    private final Vector<N3> ballInitialLinearVelocity;
    ...
    public RK4(
        ...
        this.ballInitialLinearVelocity =
            ballInitialLinearVelocityRelativeToField;
        this.ballInitialAngularVelocity =
            ballInitialAngularVelocityRelativeToField;
        this.dt = dt;
    }
    ...
    private Vector<N3> dragAcceleration(Vector<N3> v) {
        ...
        double scalar =
            (-0.5 * AIR_DENSITY * DRAG_COEFF * BALL_AREA * speed) /
            BALL_MASS_KG;
    ...
    private BallState rk4Step(Vector<N3> pos, Vector<N3> vel) {
        ...
        Vector<N3> newPos = pos.plus(
            (m1.plus(m2.times(2.0)).plus(m3.times(2.0)).plus(m4)).times(
                    dt / 6.0
                )
        );
        Vector<N3> newVel = vel.plus(
            (k1.plus(k2.times(2.0)).plus(k3.times(2.0)).plus(k4)).times(
                    dt / 6.0
                )
        );
    ...
    public BallError calculateError() {
        double[] velocityArray = {
            ballInitialLinearVelocity.get(0),
            ballInitialLinearVelocity.get(1),
            ballInitialLinearVelocity.get(2),
        };
        SmartDashboard.putNumberArray(
            "Ball Inital Linear Velocity",
            velocityArray
        );
        ...
        for (int step = 0; step < maxSteps; step++) {
            ...
            if (hasGoneAboveHub && z < Constants.hubZ) {
                SmartDashboard.putBoolean("CanReachHubZ", true);
                hasGoneAboveHub = true;
                break;
            }
            ...
            if (z < 0.0) {
                System.out.println("Hit ground before hub Z");
                SmartDashboard.putBoolean("CanReachHubZ", false);
