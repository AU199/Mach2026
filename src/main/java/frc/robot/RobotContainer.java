...
public class RobotContainer {
    ...
    private void configureBindings() {
        ...
        drivetrain.setDefaultCommand(
                ...
                drivetrain.applyRequest(() -> drive.withVelocityX(-Math.pow(controller1.getRawAxis(1), 3) * MaxSpeed) // Drive
                                                                                                                      // forward
                        // with negative Y
                        // (forward)
                        .withVelocityY(-Math.pow(controller1.getRawAxis(0), 3) * MaxSpeed) // Drive left with negative X
                                                                                           // (left)
        ...
        Pose2d targetPose = new Pose2d(1.728, 6.826, new Rotation2d(Math.PI));
        ...
        controller1.circle().onTrue(
                drivetrain.BlineToPoint(targetPose, () -> DriverStation.getAlliance().get().equals(Alliance.Red)));
        ...
        NamedCommands.registerCommand("ShootBalls",
                hood.setHoodPosition(0.09).andThen(shooter.shooterOn(60).withTimeout(2).andThen(feeder.feederOn(1))));
